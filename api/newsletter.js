...
## THE EMILY TAKE (a concise, opinionated read on what Bordeaux/Barolo/Burgundy Substack intelligence is telling us that the major critics alone wouldn't show)` },
  { key: 'vintage_watch', label: 'Vintage Watch — Emily\u2019s Take', maxTokens: 2400, instruction: `Produce a standing vintage assessment across the priority REGIONS below - this is a reference the collector checks back on, not a one-off article. This is deliberately region-level only - named producer deep-dives happen in the separate Producer Watch section, so do not try to cover both here.
...
SEARCH BUDGET: aim for roughly 4-5 searches total. Use judgement: prioritise regions most likely to have genuine news right now over mechanically re-checking everything. If a region has nothing new to report, it's fine to skip searching it this cycle and just say "no change since last issue" based on what's already known.

For each region with something genuinely current to report, give ONE clear, opinionated call - not a hedge: **PURSUE NOW**, **CELLAR FOR LATER**, **WAIT AND SEE**, or **AVOID** - with 2-3 sentences of actual reasoning citing real evidence: critic convergence across tiers, independent corroboration, growing conditions, and crucially any market/availability dynamics (e.g. importer caution due to exchange rates creating genuine scarcity, versus manufactured urgency). Keep each region's entry tight - 2-3 sentences of reasoning, not a full essay - so all six regions can realistically fit.

Be willing to say a heavily-hyped vintage isn't actually worth pursuing yet, or that an under-discussed one is - the point of this section is a real opinion, not consensus-following. If nothing has changed for a region since it was last covered, say so briefly rather than restating the same case again. Do not pad every region - skip any with nothing genuinely new to say this cycle.

If you genuinely cannot finish every region within a reasonable length, STOP CLEANLY after finishing whichever region you are on - never cut off mid-sentence. Write this as "## Vintage Watch — Emily's Take" with clear sub-headings, one per region covered.` },
  { key: 'producer_watch', label: 'Producer Watch — Emily\u2019s Take', maxTokens: 2400, instruction: `Produce a standing assessment of NAMED PRODUCERS specifically - this is a reference the collector checks back on, not a one-off article. This is deliberately producer-level only - broad regional vintage assessment happens in the separate Vintage Watch section, so do not try to cover both here.

NAMED PRODUCERS TO CHECK (in addition to the full watchlist provided below):
- Barolo/Piedmont: Massolino, Paolo Scavino, Gaja (technically Barbaresco - note this distinction if it comes up rather than blurring it), Monchiero.
- Burgundy: Domaine Dujac, Marc Roy, Lignier-Michelot, Faiveley, L'Arlot (Tier 2, realistically accessible with the right introduction) - and Domaine Armand Rousseau, Mugnier (Tier 1, essentially closed to cold approaches, but still worth tracking for any genuine news - a new release, a critic score, a change in availability).
Also actively search for lesser-known, currently-excellent producers in these regions that aren't on the list yet - the collector wants genuine discoveries here, not just the famous names repeated back.

SEARCH BUDGET: aim for roughly 4-5 searches total. Prioritise the 2-3 producers most likely to have genuine news this cycle and cover them properly; the rest can get a brief "no change since last update" if nothing new turns up. Covering all 9+ named producers every single cycle is neither necessary nor realistic within a reasonable length.

For each producer with something genuinely current to report, give ONE clear, opinionated call - not a hedge: **PURSUE NOW**, **CELLAR FOR LATER**, **WAIT AND SEE**, or **AVOID** - with 2-3 sentences of actual reasoning citing real evidence: critic convergence across tiers, independent corroboration, and crucially any market/availability dynamics. Keep each producer's entry tight so multiple producers can realistically fit in one response.
...
Be willing to say a heavily-hyped producer isn't actually worth pursuing yet, or that an under-discussed one is - the point of this section is a real opinion, not consensus-following. If nothing has changed for a producer since it was last covered, say so briefly rather than restating the same case again.

If you genuinely cannot finish every producer within a reasonable length, STOP CLEANLY after finishing whichever producer you are on - never cut off mid-sentence. Write this as "## Producer Watch — Emily's Take" with clear sub-headings, one per producer covered, and end with a short "Where to Buy" list of the specific Australian sources that came up.` },
]
...
export function buildHandler({ createClient: createClientDep, fetchImpl }) {
  ...
  try {
    ...
    if (sectionDef.recordLeads) {
      try {
        const extraction = await extractLeadsFromText({ fetchImpl, apiKey: process.env.ANTHROPIC_API_KEY, text, sectionLabel: sectionDef.label })
        if (extraction.ok) {
          ...
          console.warn('Wine Radar: lead extraction failed:', extraction.error)
        }
      } catch (e) {
