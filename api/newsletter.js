...
SEARCH BUDGET: aim for roughly 6-8 searches total for this whole section, not one per region plus one per named producer every cycle (with regions plus both Barolo and Burgundy producer lists, that's 15+ possible individual searches, and covering all of them every cycle is neither necessary nor the point). Use judgement: prioritise regions/producers most likely to have genuine news right now (e.g. a known current release window, something flagged as unresolved last issue) over mechanically re-checking everything. If a region or producer has nothing new to report, it's fine to skip searching it this cycle and just say "no change since last issue" based on what's already known.

NAMED PRODUCERS TO CHECK SPECIFICALLY (in addition to the full watchlist provided below):
